...
from fly_dask.sdk.exceptions import (
    AppInterfaceError,
    AppNotFound,
    MachineInterfaceError,
    MissingMachineIdsError,
)
from fly_dask.sdk.models.apps import (
    FlyAppCreateRequest,
...
class Fly:
    ...
    async def create_app(
        ...
        r = await self._make_api_post_request(path, app_details.model_dump())

        if r.status_code == 404:
            raise AppNotFound(message=f"Organization {org_slug} not found!")
    ...
    async def is_app_exists(self, app_name: str) -> bool:
        return await self.try_get_app(app_name) is not None

    async def try_get_app(
        self,
        app_name: str,
    ) -> FlyAppDetailsResponse | None:
    ...
    async def get_app(
        ...
        if r.status_code == 404:
            raise AppNotFound(message=f"App {app_name} not found!")
    ...
    async def create_machine(
        ...
        r = await self._make_api_post_request(
            path,
            payload=machine_create_request.model_dump(exclude_defaults=True),
        )
    ...
    async def delete_machines(
        self,
        app_name: str,
        machine_ids: list[str] | list[FlyMachineDetails] = [],
        ...
        for machine in machine_ids:
            await self.stop_machine(
                app_name, machine if isinstance(machine, str) else machine.id
            )

        # Delete machines.
        for machine in machine_ids:
            await self.delete_machine(
                app_name,
                machine.id if isinstance(machine, FlyMachineDetails) else machine,
                force=force,
            )
    ...
    async def _make_api_delete_request(
        ...
        async with httpx.AsyncClient(verify=False, timeout=None) as client:
            return await client.delete(url, headers=self._generate_headers())
